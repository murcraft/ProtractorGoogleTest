...
class BaseElement extends BasePage {
  ...
  async waitForIsNotPresent (timeoutMs = timeout) {
    try {
      await browser.wait(EC.stalenessOf(this.element), timeoutMs)
    }
    catch (e) {
      await Logger.error(`Element '${this.name}' with locator ${this.by} is still present after ${timeoutMs / 1000} sec`)
      throw `Element '${this.name}' with locator ${this.by} is still present after ${timeoutMs / 1000} sec`
    }
  }

  async mouseOverElement () {
    await this.waitForIsPresent()
    await this.waitForIsClickable()
    await Logger.debug(`${this.name} :: Hovering mouse`)
    await browser.actions().mouseMove(this.element).perform()
  }
