...
describe('Suite3 - Search Buildings from 35000 to 40000: ', () => {
  ...
  it(`check building in Buildings Typeahead`, async () => {
    if (buildingsArr.length > checkedBuildingsNumber) {
      for (let n = initialCheckedNumber; n < checkedBuildingsNumber; n++) {
        ...
        expect(displayAddressesArray).toContain(buildingAddress.toUpperCase().trim(), `Building ${n} with address ${buildingAddress} is absent in Search Buildings response`)
