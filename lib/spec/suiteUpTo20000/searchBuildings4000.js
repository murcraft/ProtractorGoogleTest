...
describe('Search Buildings To 20000 - Search Buildings to 4000: ', () => {

  let randomText = RandomHelper.GetRandomString('log')
  let typeaheadFileName = `typeahead_search4000_${randomText}.txt`

  let buildingsArr = []
  let buildingAddress
  let buildingId
  let initialCheckedNumber = 0
  let checkedBuildingsNumber = 4000
  let recordedFile
  ...
  it('get buildings from Buildings in Search and number of checked buildings', async () => {
    ...
    if (buildingsArr.length > initialCheckedNumber) {
      ...
      await LoggerInfra.Debug(`Writing logs in ${typeaheadFileName} file`)
