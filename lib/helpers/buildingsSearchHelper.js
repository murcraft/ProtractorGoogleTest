...
class BuildingsSearchHelper {
  ...
  static async GetBuildingsAndIdArr (user) {
    ...
    if (response !== undefined) {
      ...
      await LoggerInfra.Debug(`Waiting for file 'buildings.txt' to be writing...`)
      let arrElement = ''
      response.forEach((data) => {
        arrElement += data + '\n'
      })
      currentTime = RandomHelper.GetDateTimeString()
      await BasePage.WriteInNewFile(`[${currentTime}] - Getting ${response.length} buildings for user '${user.email}'\n` + arrElement, 'buildings.txt')
    } else {
      currentTime = RandomHelper.GetDateTimeString()
      await BasePage.WriteInNewFile(`[${currentTime}] - Getting ${response} buildings for user '${user.email}'\n`, 'buildings.txt')
    }
    return response
  }

  static async GetBuildingsInfoForAddress (user, buildingArr, number, fileName) {
    ...
    await BasePage.WriteInExistFile(`[${currentTime}] - Index number ${number} Getting response in Building typeahead for '${buildingAddress}', id is '${buildingId}'\n`, fileName)
    let appApiClient = new AppApiClient(user)
    await appApiClient.authenticate(true)
