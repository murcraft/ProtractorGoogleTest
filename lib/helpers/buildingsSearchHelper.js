...
class BuildingsSearchHelper {
  ...
  static async GetBuildingsInfoForAddress (user, buildingArr) {
    let buildingAddress = buildingArr[0].trim()
    let buildingId = buildingArr[1]
    await LoggerInfra.Debug(`Getting response in Building typeahead for '${buildingAddress}', id is '${buildingId}'`)
