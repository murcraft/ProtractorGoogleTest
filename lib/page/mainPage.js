...
class MainPage extends BasePage {
  ...
  static async SignOut () {
    ...
    if (BrowserName === 'safari') {
      ...
      child_process.execSync(`screencapture -t jpg artifacts + ${tm}.jpg`)
      ...
      child_process.execSync(`screencapture -t jpg artifacts + ${tm + 1}.jpg`)
      await browser.get('/accounts/sign_out')
      await Logger.debug('Get accounts/sign_out')
      child_process.execSync(`screencapture -t jpg artifacts + ${tm + 2}.jpg`)
    } else {
      await browser.get('/accounts/sign_out')
    }
    await new BasePage().waitForLoadPage()
