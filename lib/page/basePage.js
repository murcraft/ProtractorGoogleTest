...
class BasePage {
    ...
    Logger.debug('Wait for page load: ' + link)
     ...
     try {
      ...
      try {
        ...
        Logger.debug('Cannot get page source', e.message)
      }

      await Logger.error(`Page is not loaded within ${timeout / 1000} sec`, e)
      throw `Page is not loaded within ${timeout / 1000} sec \n` + e
    }
    try {
  ...
  async printPageSource () {
    ...
    await Logger.info('Page source:')
    await Logger.info(pageSource)
  }
