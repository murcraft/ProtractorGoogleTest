...
class BasePage {
  ...
  async waitForLoadPage () {
    try {
      await browser.wait(protractor.ExpectedConditions.alertIsPresent(), 5000).catch(async (e) => {
      })
      try {
        await browser.switchTo().alert().accept()
        await Logger.debug(`Accepted alert`)
      } catch (e) {
        await Logger.debug('Alert is not accepted')
        throw 'Alert is not accepted' + e
      }
    }
    catch (e) {
    }
