...
class BasePage {
  ...
  static async ReadFile (fileName) {
    let pathFile = await path.resolve(readDir, fileName)
    await Logger.debug(`Waiting for file ${pathFile} to be reading`)
  ...
  static async ReadLinesInFile (fileName, callback) {
    let pathFile = await path.resolve(readDir, fileName)
    let buildingsArray = []
    ...
    rl.on('line', (line) => {
      buildingsArray.push(line)
    })

    rl.on('close', () => {
      callback(buildingsArray)
    })
  }
